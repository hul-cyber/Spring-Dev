...
Spring为了同时支持`JDBC`和`JTA`两种事务模型，就抽象出`PlatformTransactionManager`。因为我们的代码只需要JDBC事务，因此，在`AppConfig`中，需要再定义一个`PlatformTransactionManager`对应的Bean，它的实际类型是`DataSourceTransactionManager`：
```java
@Configuration
@ComponentScan
...
```
可见，`BonusService.addBonus()`方法必须要有`@Transactional`，否则，登录后积分的添加就受不到事务的保证了。
...
`NESTED`：表示如果当前有事务，则开启一个嵌套级别事务，如果当前没有事务，则开启一个新事务。(**嵌套事务是指在一个已经启动的事务的范围内，由一条指令启动的数据库事务。嵌套事务在不同的数据库中的实现方式不同。然而，它们的共同点是，在最外层的事务提交之前，变化对任何不相关的事务都不可见。这意味着内部事务中的提交不一定会将更新持久化到系统中。**)
`NO_SUPPORTS`与`REQUIRES_NEW`之间的区别：
对于下面的代码：
```java
class A {
    public void invoke() {
